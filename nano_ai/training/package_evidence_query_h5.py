"""Build a deterministic, authenticated RunPod bundle for Nano H5.

The bundle contains only the frozen H5 dataset, executable authorities, base
checkpoint, tokenizer, and pinned RunPod requirements.  Its first script
...
H5_RUNPOD_BUNDLE_SCHEMA_VERSION = "nano.h5-runpod-bundle.v1"
H5_READINESS_SCHEMA_VERSION = "nano.h5-readiness.v1"
...
BUNDLE_FILENAME = "nano-h5-runpod-input.tar.gz"
READINESS_FILENAME = "READINESS.json"
ARCHIVE_ROOT = "nano-h5-runpod"

# Exact transitive H5 runtime and focused-test inventory.  Deliberately use an
...
_SOURCE_ALLOWLIST = (
    ...
    "nano_ai/training/evaluate_evidence_query_h5.py",
    ...
    "nano_ai/training/package_evidence_query_h5.py",
    ...
    "nano_ai/training/train_evidence_query_h5.py",
    "nano_ai/training/train_pointer.py",
    "nano_ai/training/train_state_span.py",
    "nano_ai/tests/test_evaluate_evidence_query_h5.py",
    "nano_ai/tests/test_package_evidence_query_h5.py",
    "nano_ai/tests/test_replay_mixture_data.py",
    "nano_ai/tests/test_train_evidence_query_h5.py",
    "pyproject.toml",
    "requirements-h4-runpod.txt",
)


class H5PackagingError(RuntimeError):
    """Raised when an H5 bundle cannot be proven reproducible."""
...
def _read_regular_file(path: Path, *, role: str) -> bytes:
    try:
        ...
        raise H5PackagingError(f"{role} is unavailable or not a regular file") from exc
...
def _require_digest(path: Path, expected: str, *, role: str) -> dict[str, Any]:
    ...
    if identity["sha256"] != expected:
        raise H5PackagingError(f"{role} SHA-256 mismatch")
    return identity
...
test ! -e .h5-venv
test "$(sha256sum h5_data/manifest.json | awk '{{print $1}}')" = "{training_manifest_sha256}"
python -m venv --system-site-packages .h5-venv
...
python -m pytest -q nano_ai/tests/test_replay_mixture_data.py nano_ai/tests/test_train_evidence_query_h5.py nano_ai/tests/test_evaluate_evidence_query_h5.py nano_ai/tests/test_package_evidence_query_h5.py

test ! -e h2_development
mkdir results
python -m nano_ai.training.train_evidence_query_h5 --data-dir h5_data --base-checkpoint checkpoints/anchors/nano_v01_scribe.pt --tokenizer sft/tokenizer.json --output-dir results/seed-{seed_a} --seed {seed_a} --device cuda 2>&1 | tee results/seed-{seed_a}.log
test ! -e h2_development
python -m nano_ai.training.train_evidence_query_h5 --data-dir h5_data --base-checkpoint checkpoints/anchors/nano_v01_scribe.pt --tokenizer sft/tokenizer.json --output-dir results/seed-{seed_b} --seed {seed_b} --device cuda 2>&1 | tee results/seed-{seed_b}.log
test ! -e h2_development
...
"""
    if training_manifest_sha256 not in script:
        raise AssertionError("training manifest pin is missing from H5 training script")
...
def _evaluation_run_script(*, training_manifest_sha256: str) -> bytes:
    ...
    prelude = _termination_prelude(
        temporary_paths=(
            "SHA256SUMS.tmp",
            "nano-h5-results.tar.gz.tmp",
            "nano-h5-results.tar.gz.sha256.tmp",
        )
    )
    script = f"""#!/usr/bin/env bash
set -euo pipefail
...
python -m nano_ai.training.evaluate_evidence_query_h5 --training-data-dir h5_data --training-manifest-sha256 {training_manifest_sha256} --development-data-dir h2_development --development-manifest-sha256 {H2_DEVELOPMENT_MANIFEST_SHA256} --tokenizer sft/tokenizer.json --training-report results/seed-{seed_a}/training_report.json "$report_a_sha" --training-report results/seed-{seed_b}/training_report.json "$report_b_sha" --output results/development_evaluation.json --device cuda --batch-size 32 2>&1 | tee results/development_evaluation.log
    ...
    with (
        ...
        for name, payload in sorted({**members, **generated}.items()):
            _add_bytes(
                ...
                mode=(
                    0o755
                    if name in {"RUN_H5_TRAIN.sh", "RUN_H5_EVALUATE.sh"}
                    else 0o644
                ),
            )
    ...
    try:
        with tarfile.open(fileobj=io.BytesIO(archive_bytes), mode="r:gz") as archive:
            ...
            if len(observed_names) != len(set(observed_names)):
                raise H5PackagingError("H5 archive contains duplicate members")
            if set(observed_names) != expected_names:
                raise H5PackagingError("H5 archive member inventory mismatch")
            for entry in entries:
                if not entry.isfile() or entry.name.startswith("/"):
                    raise H5PackagingError("H5 archive contains an unsafe member")
                ...
                if payload_file is None:
                    raise H5PackagingError("H5 archive member is unreadable")
                ...
                if (
                    ...
                    raise H5PackagingError(
                        f"H5 archive member identity mismatch: {relative}"
                    )
    except (OSError, tarfile.TarError, ValueError) as exc:
        raise H5PackagingError("H5 archive could not be authenticated") from exc


def package_h5_runpod_bundle(
    *,
    repo_root: str | Path,
    data_dir: str | Path,
    output_dir: str | Path,
) -> Mapping[str, Any]:
    """Validate and package the exact H5 input without overwriting artifacts."""

    root = Path(repo_root).resolve()
    data = Path(data_dir).resolve()
    output = Path(output_dir).resolve()
    if output.exists():
        raise H5PackagingError("H5 packaging output directory already exists")
    if not output.parent.is_dir():
        raise H5PackagingError("H5 packaging output parent must exist")
    ...
    paths.update(
        {
            ...
            "h5_data/manifest.json": data / "manifest.json",
            "h5_data/fit.jsonl": data / "fit.jsonl",
            "h5_data/calibration.jsonl": data / "calibration.jsonl",
        }
    )
    member_payloads = {
    ...
    for name, payload in (
        ("RUN_H5_TRAIN.sh", training_script),
        ("RUN_H5_EVALUATE.sh", evaluation_script),
    ):
    ...
    bundle_manifest = {
        "schema_version": H5_RUNPOD_BUNDLE_SCHEMA_VERSION,
        ...
        "expected_runtime": {
            ...
            "isolated_environment": ".h5-venv with system-site-packages",
        },
    ...
    readiness = {
        "schema_version": H5_READINESS_SCHEMA_VERSION,
        ...
        "inputs": {
            "training_manifest": {
                "bytes": len(
                    _read_regular_file(data / "manifest.json", role="H5 manifest")
                ),
        ...
        "training_seeds": list(TRAINING_SEEDS),
        "training_command": (
            "tar -xzf nano-h5-runpod-input.tar.gz && cd nano-h5-runpod "
            "&& ./RUN_H5_TRAIN.sh"
        ),
        "evaluation_command_after_development_transfer": (
            "cd nano-h5-runpod && ./RUN_H5_EVALUATE.sh"
        ),
    ...
    try:
        ...
        raise H5PackagingError("H5 packaging outputs could not be created") from exc
    return readiness


def _parser() -> argparse.ArgumentParser:
    root = Path(__file__).resolve().parents[2]
    parser = argparse.ArgumentParser(description="Package Nano H5 for RunPod")
...
__all__ = [
    "BUNDLE_FILENAME",
    "H5_READINESS_SCHEMA_VERSION",
    "H5_RUNPOD_BUNDLE_SCHEMA_VERSION",
    "READINESS_FILENAME",
    "H5PackagingError",
    "package_h5_runpod_bundle",
]

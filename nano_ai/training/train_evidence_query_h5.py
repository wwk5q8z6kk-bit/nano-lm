"""Frozen H5 replay-mixture trainer for Nano's evidence-query scribe.

H5 changes only the gradient-bearing fit-family composition: half regenerated
legacy H3 worlds and half H4 surface-transfer worlds.  It reuses the unchanged
H4 training-only calibration partition.  Architecture, tokenizer, prompt,
objective, optimizer, schedule, exposure, seeds, threshold policy, and
selection order remain frozen.  This module never opens development, benchmark,
...
H5_TRAINING_REPORT_SCHEMA_VERSION = "nano.evidence-query-h5-training-report.v1"
H5_TRAINING_RECIPE_VERSION = replay_mixture_data.TRAINING_RECIPE_VERSION
...
_TRAINING_SELECTION_NOTE = (
    "The selected H5 epoch used only the unchanged H4 200-world "
    "training-calibration partition. Gradients used only the fixed 1,400-world "
    "legacy plus 1,400-world surface replay fit mixture. The training command "
    "read no legacy training-record artifact, development, benchmark, "
    "historical-fresh, or sealed-confirmation records. H3/H4 architecture, "
    "objective, optimizer, exposure, seeds, inference, calibration, and "
    "selection rules were preserved by literal source and input pins."
)
...
def changed_source_paths() -> Mapping[str, Path]:
    """Return the two H5-specific executable authorities."""

    return {
        "data_generator": Path(replay_mixture_data.__file__).resolve(),
...
def _require_preserved_h3_sources() -> dict[str, str]:
    ...
    if observed != dict(PRESERVED_H3_SOURCE_SHA256):
        ...
        raise TrainingInputError(
            "H5 preserved H3 source hash mismatch: " + ", ".join(changed)
        )
    return observed


def _changed_source_hashes() -> dict[str, str]:
    return _hash_paths(changed_source_paths(), label="changed H5")
...
def _require_unchanged_sources(changed_expected: Mapping[str, str]) -> None:
    ...
    if _changed_source_hashes() != dict(changed_expected):
        raise TrainingInputError("H5 changed source changed during execution")
...
def _require_partition_state_distributions(
    ...
    if fit_counts != FIT_STATE_CLASS_COUNTS:
        raise TrainingInputError("H5 fit state-class distribution drifted")
    if calibration_counts != CALIBRATION_STATE_CLASS_COUNTS:
        raise TrainingInputError("H5 calibration state-class distribution drifted")
    if (
        tuple(
            fit + calibration
        ...
        raise RuntimeError("H5 partitions do not reconstruct frozen class counts")
...
def _require_exact_training_steps(
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if len(batches) != STEPS_PER_EPOCH or any(
            ...
            raise TrainingInputError("H5 exposure is not exactly 350 full batches")
        flattened = [index for batch in batches for index in batch]
        if sorted(flattened) != list(range(FIT_RECORD_COUNT)):
            raise TrainingInputError("H5 epoch does not expose each fit record once")
        batches_by_epoch[epoch] = batches
    if sum(len(value) for value in batches_by_epoch.values()) != STEPS_PER_SEED:
        raise TrainingInputError("H5 exposure is not exactly 1,050 steps per seed")
...
def _require_input_hashes(
    *,
    ...
    if observed != dict(expected):
        raise TrainingInputError("H5 authenticated input changed during execution")
...
def train_evidence_query_h5_candidate(
    *,
    data_dir: Path,
    base_checkpoint: Path,
    tokenizer_path: Path,
    ...
    """Train one H5 seed after complete fail-closed, no-output preflight."""
    ...
    if base_sha256 != FROZEN_NANO_V01.checkpoint_sha256:
        raise TrainingInputError("H5 base checkpoint identity drifted")
    if tokenizer_sha256 != FROZEN_NANO_V01.tokenizer_sha256:
        raise TrainingInputError("H5 tokenizer identity drifted")
    ...
    if (
        ...
        raise RuntimeError("H5 model identity drifted before training")
    ...
    if global_step != STEPS_PER_SEED:
        raise RuntimeError("H5 completed with an unexpected optimizer-step count")
    ...
    if set(dataset_report) != REPORT_DATASET_KEYS:
        raise RuntimeError("H5 training dataset report contract drifted")
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Train Nano's frozen H5 replay-mixture candidate"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    train_evidence_query_h5_candidate(

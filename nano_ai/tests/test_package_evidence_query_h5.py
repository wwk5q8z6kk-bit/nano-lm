...
from nano_ai.training import package_evidence_query_h5


def test_h5_packaging_contract_is_versioned_and_not_eager() -> None:
    assert (
        package_evidence_query_h5.H5_RUNPOD_BUNDLE_SCHEMA_VERSION
        == "nano.h5-runpod-bundle.v1"
    )
    assert (
        package_evidence_query_h5.H5_READINESS_SCHEMA_VERSION == "nano.h5-readiness.v1"
    )
    assert package_evidence_query_h5.BUNDLE_FILENAME == ("nano-h5-runpod-input.tar.gz")
    assert package_evidence_query_h5.READINESS_FILENAME == "READINESS.json"


def test_archive_bytes_are_deterministic_and_normalized() -> None:
    members = {"z.txt": b"last\n", "a.txt": b"first\n"}
    generated = {"RUN_H5_TRAIN.sh": b"#!/usr/bin/env bash\ntrue\n"}

    first = package_evidence_query_h5._archive_bytes(members, generated)
    second = package_evidence_query_h5._archive_bytes(members, generated)
    ...
    with (
        ...
        assert [entry.name for entry in entries] == [
            "nano-h5-runpod/RUN_H5_TRAIN.sh",
            "nano-h5-runpod/a.txt",
            "nano-h5-runpod/z.txt",
        ]
...
def test_archive_self_verification_authenticates_every_member() -> None:
    ...
    generated = {
        ...
        "RUN_H5.sh": b"#!/usr/bin/env bash\ntrue\n",
    }
    archive = package_evidence_query_h5._archive_bytes(members, generated)
    identities = {
        name: {
            "bytes": len(payload),
        ...
        for name, payload in {
            **members,
            "RUN_H5.sh": generated["RUN_H5.sh"],
        }.items()
    }

    package_evidence_query_h5._verify_archive_bytes(
        archive,
    ...
    with pytest.raises(
        package_evidence_query_h5.H5PackagingError,
...
def test_archive_rejects_generated_member_collision() -> None:
    with pytest.raises(
        ...
        package_evidence_query_h5._archive_bytes(
            {"RUN_H5.sh": b"input"},
            {"RUN_H5.sh": b"generated"},
        )


def test_run_scripts_freeze_training_before_development_and_evaluation() -> None:
    manifest_sha256 = "a" * 64
    training = package_evidence_query_h5._training_run_script(
        training_manifest_sha256=manifest_sha256
    ).decode("utf-8")
    evaluation = package_evidence_query_h5._evaluation_run_script(
        training_manifest_sha256=manifest_sha256
    ).decode("utf-8")

    assert training.count("train_evidence_query_h5 --data-dir") == 2
    assert "--seed 20260805" in training
    assert "--seed 20260806" in training
    assert "python -m nano_ai.training.evaluate_evidence_query_h5" not in training
    ...
    assert "python -m nano_ai.training.evaluate_evidence_query_h5" in evaluation
    assert "--training-manifest-sha256 " + manifest_sha256 in evaluation
    assert evaluation.count("--training-report") == 2
    assert package_evidence_query_h5.H2_DEVELOPMENT_MANIFEST_SHA256 in evaluation
    assert package_evidence_query_h5.H2_DEVELOPMENT_SHA256 in evaluation
...
def test_run_scripts_pin_matched_runtime_and_are_terminate_safe() -> None:
    ...
    assert "python -m venv --system-site-packages .h5-venv" in training
    assert 'export PATH="$PWD/.h5-venv/bin:$PATH"' in training
    assert 'export PATH="$PWD/.h5-venv/bin:$PATH"' in evaluation
    ...
    assert "nano-h5-results.tar.gz.tmp" in evaluation
...
def test_packager_rejects_existing_output_before_reading_inputs(
    ...
    with pytest.raises(
        ...
        package_evidence_query_h5.package_h5_runpod_bundle(
...
def test_bundle_source_inventory_is_an_exact_safe_allowlist(tmp_path: Path) -> None:
    members = package_evidence_query_h5._source_members(tmp_path)

    assert set(members) == set(package_evidence_query_h5._SOURCE_ALLOWLIST)

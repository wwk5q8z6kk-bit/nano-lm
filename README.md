...
| **Program state** | **Active:** `frontier/active-v1` / Nano Runtime Wedge v1 (`papers/PROGRAM_AUTHORITY.md`). Paper α frozen (`paper-alpha-v1`). Post-α E4 Gate 4 KILL archived outside Paper α (`trajectory/results_e4_utility.json`); further E4 execute BLOCKED. No E2/fabric/NanoScribe expansion. Small-LM training not default. |
...
(**≠** NanoScribe). After E1 KILL: product/architecture expansion **STOP** for the old scribe thesis; E3
agent-rubric audit done (**EXACT_SURVIVES**; dual-clinician IAA open — not human
eval); E2 **GATED/STOP** (no results JSON). Post-α R★/E4 Gate 4 **KILL** is archived
in `trajectory/results_e4_utility.json` (outside Paper α); further E4 execute **BLOCKED**.
Active engineering: Nano Runtime Wedge v1 on `frontier/active-v1` (see `papers/PROGRAM_AUTHORITY.md`).

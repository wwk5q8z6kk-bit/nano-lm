...
def manipulation_check(m, items):
    ...
    for it in items:
        ...
        logP,p_gen,pcopy,cshare=m.logprob_at_targets(x,tgt,srcv)[:4]
...
if __name__ == "__main__":
    ...
    if arm == "baseline":
        ...
        m = GPTCopy2(); m.load_state_dict(torch.load(ckpt, map_location="cpu", weights_only=True)); m.to(dev).eval()

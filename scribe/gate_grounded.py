# Stage G — grounding-verifier guardrail, single measurement run.
# Bars PRE-REGISTERED in AUDIT.md (committed to git before this script ran):
#   residual hallucination (among PRESENTED fields) <= 2.5%
#   presented-field precision >= 95%
#   review load (flagged) <= 25%
# Verifier: value=="none" -> present; else present iff value is a case-insensitive
# substring of a PATIENT utterance; otherwise FLAG for human review.
...
for it in items:
    ...
    out = generate(prompt_ids(it["convo"][0]["content"]))
    text = tok.decode(out[len(prompt_ids(it["convo"][0]["content"])):]).strip()
    mm = RE.match(text)
    total += 5
    if not mm:
        unparsed += 1
        flagged += 5                      # unparseable draft -> whole item to review
        continue
    ...
    for f in FIELDS:
        ...
        grounded = (p == "none") or (p.lower() in ptext)
        if grounded:
            presented += 1
            if p == t: pres_correct += 1
            elif p == "none" and t != "none": pres_omission += 1
            else:
                pres_halluc += 1
                survivors.append((f, p, t))
        else:
            flagged += 1
...
print(f"verifier catch-rate on hallucinations: {halluc_caught}/{halluc_total} = {halluc_caught/max(1,halluc_total):.0%}", flush=True)
print(f"recall among all fields (presented & correct): {pres_correct}/{total} = {pres_correct/total:.1%}", flush=True)
if survivors:
    print("surviving hallucinations (wrong-field-right-source class, as predicted in AUDIT):", flush=True)
    for f, p, t in survivors[:8]:
        print(f"  field {f}: presented {p!r}  truth {t!r}", flush=True)

print("\n--- verdict on pre-registered Stage-G bars ---", flush=True)
...
print("STAGE G PASS" if (b1 and b2 and b3) else "STAGE G FAIL", flush=True)

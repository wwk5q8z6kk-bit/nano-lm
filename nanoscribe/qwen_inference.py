...
_SPAN_PORT_SYSTEM = (
    ...
    "For STATED or DENIED include a verbatim quote from the transcript in double quotes. "
    'Examples: STATED: "neck"  DENIED: "No allergies."  NOT_MENTIONED'
)
...
def _format_transcript(model_input) -> str:
    if model_input.prompt:
        return model_input.prompt
    lines: list[str] = []
    for turn in model_input.source.turns:
        lines.append(f"{turn.speaker.value}: {turn.text}")
    return "\n".join(lines)


def _atom_user_prompt(spec: AtomSpec, transcript: str) -> str:
    return (
        f"Transcript:\n{transcript}\n\n"
        f"Slot: {spec.atom_id}\n"
        f"Type: {spec.atom_type.value}\n"
        f"Value hint: {spec.raw_value}\n"
        "One-line answer:"
    )
...
def _generate_line(loaded: _LoadedQwen, user_prompt: str, *, max_new_tokens: int) -> str:
    ...
    messages = [
        {"role": "system", "content": _SPAN_PORT_SYSTEM},
...
def generate_span_port_lines(
    ...
    *,
    ...
    transcript = _format_transcript(model_input)
    lines: dict[str, str] = {}
    for spec in atom_specs:
        raw = _generate_line(
            loaded,
            _atom_user_prompt(spec, transcript),
            max_new_tokens=max_new_tokens,
        )

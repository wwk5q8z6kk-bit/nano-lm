...
> ## ⚠ CORRECTION NOTICE — 2026-08-25, added after publication
>
...
> place" identically to "found the right place, wrong extent". Splitting them:
> the model selects the **correct conversational turn** for 95/120 gold-bearing
> slots (79%) while delimiting the gold span for only 16/120 (13%). Located
> quotes are turn-scale — median 29 chars against gold median 8, median
> quote/enclosing-turn ratio **1.000**, 1 of 95 exceeding its turn. Across the
> question-form change in correction 1, exact-extent moves 16→2 while LOCATED
> moves 95→97. The bottleneck is **delimitation, not retrieval**.
>
> **What is NOT corrected — §4's C1 verdict stands.** It was checked for
> cross-form confounding and is clean (`git diff --stat 7447df5 a91ce6b` touches
> only `nanoscribe/leakage.py`; `prompt.py` is byte-identical), and its verdict
> survives an exact McNemar on the per-slot data: grounded b=16/c=2 p=1.3e-3,
> unbound b=38/c=6 p=9.4e-7, co-movement fires → **UNRESOLVED — coverage shift**.
> The reported +1.17 / +2.67 / −1.08 reproduce exactly. Only the interval *shape*
> should change (paired *t* at df=11 → exact discordant counts).
>
> Full re-analysis: `artifacts/RESULT-span-extent-reanalysis.md`.

...
TRAIN_LOOP='for rid in '"${RUN_IDS[*]}"'; do python3 scripts/train_native_nano.py --run-id "$rid" || exit 1; done'
REMOTE="python3 scripts/train_native_nano.py --export-train-json; ${TRAIN_LOOP}; mkdir -p /workspace/campaign_native_checkpoints && rsync -a artifacts/native_checkpoints/ /workspace/campaign_native_checkpoints/; echo NATIVE100_DONE"
...
POD_JSON=$(runpodctl pod create \
  ...
  --ports "22/tcp" \
  --docker-args "bash -lc $(printf %q "${SYNC_CMD}; python3 -m nanoscribe.runpod_gpu_preflight || exit 1; ${REMOTE}")")

...
NATIVE_REMOTE='python3 scripts/train_native_nano.py --export-train-json; for rid in native30_bottleneck_span_s0 native30_bottleneck_span_s1 native30_bottleneck_struct_s0 native30_bottleneck_struct_s1 native30_decoder_span_s0 native30_decoder_span_s1 native30_decoder_struct_s0 native30_decoder_struct_s1; do python3 scripts/train_native_nano.py --run-id "$rid" || exit 1; done; echo NATIVE_DONE'
...
launch_native_split() {
  local half1='python3 scripts/train_native_nano.py --export-train-json; for rid in native30_bottleneck_span_s0 native30_bottleneck_span_s1 native30_bottleneck_struct_s0 native30_bottleneck_struct_s1; do python3 scripts/train_native_nano.py --run-id "$rid" || exit 1; done; echo NATIVE_HALF1_DONE'
  local half2='python3 scripts/train_native_nano.py --export-train-json; for rid in native30_decoder_span_s0 native30_decoder_span_s1 native30_decoder_struct_s0 native30_decoder_struct_s1; do python3 scripts/train_native_nano.py --run-id "$rid" || exit 1; done; echo NATIVE_HALF2_DONE'
  TS=$(date +%Y%m%d%H%M)

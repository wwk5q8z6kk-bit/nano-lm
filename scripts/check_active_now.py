...
SYNCED = (
    ...
    "training_backend",
    "training_status",
    "local_zero_cost_exploratory_training",
...
def main() -> int:
    ...
    table = parse_table(MD.read_text())
    ...
    if set(table) - set(SYNCED):
        errs.append(f"extra md keys {sorted(set(table) - set(SYNCED))}")
    for k in REQ:
        if k not in data:
            errs.append(f"json missing required {k}")
    if not isinstance(data.get("owner_gates"), list) or not data["owner_gates"]:
        errs.append("owner_gates empty")
    base = str(data.get("integration_base_sha", ""))
    if not base or base in BAD:
        errs.append(f"bad integration_base_sha {base!r}")
    expected = {
    ...
    for key, value in expected.items():
        if data.get(key) != value:
            errs.append(f"{key} must be {value}")
    if errs:

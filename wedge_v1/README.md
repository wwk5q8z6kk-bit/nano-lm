...
python -m wedge_v1 ask --escalate-stub "invalidation window for memoized responses"
python -m wedge_v1 --corpus papers find "0.925"
python -m wedge_v1 ask --doc tech_note_cache "TTL?"
python -m wedge_v1 scan
```

**Latest:** dogfood **8/8** on `papers/` with fail-closed abstention. `--escalate-stub` is opt-in (default off); `WEDGE_ESCALATE_STUB=1` also enables it.

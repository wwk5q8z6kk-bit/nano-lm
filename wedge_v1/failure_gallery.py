"""Failure gallery exporters grouped into decision-useful buckets.
...
PUBLIC_GALLERY_JSON = ROOT / "results_wedge_v1_failure_gallery.json"
PUBLIC_GALLERY_MD = ROOT / "results_wedge_v1_failure_gallery.md"
OWNER_GALLERY_JSON = ROOT / "results_owner_failure_gallery.json"
OWNER_GALLERY_MD = ROOT / "results_owner_failure_gallery.md"
...
def build_gallery(
    ...
    return {
        "schema": "nano-lm.wedge_v1.failure_gallery.v1",
        "source": str(path),
        "corpus_class": dogfood.get("corpus_class"),
        ...
        "rows": rows_out,
        "note": (
            "Wedge engineering failure gallery; not Layer-1. "
...
def _owner_private_source(
    path: Path | None,
    *,
    _seen: set[Path] | None = None,
    _depth: int = 0,
) -> bool:
    """Identify private owner inputs, including galleries derived from them."""
    if path is None:
        return False
    if _depth > 8:
        return True
    source = Path(path)
    if source.name.startswith("results_owner_"):
        return True
    try:
    ...
    corpus_class = str(raw.get("corpus_class") or "") if isinstance(raw, dict) else ""
    if "owner_dogfood_result" in schema or corpus_class == "OWNER_PRIVATE":
        return True

    # A custom-named gallery can point to a private owner-dogfood artifact. Follow
    # that local provenance rather than losing the privacy label on re-export.
    nested_source = raw.get("source") if isinstance(raw, dict) else None
    if isinstance(nested_source, str) and nested_source.strip():
        nested = Path(nested_source).expanduser()
        candidates = (
            [nested] if nested.is_absolute() else [source.parent / nested, nested]
        )
        existing = [candidate for candidate in candidates if candidate.is_file()]
        if not existing:
            return True
        return any(
            _owner_private_source(candidate, _seen=seen, _depth=_depth + 1)
...
def write_gallery(
    ...
    *,
    ...
    source = Path(path) if path is not None else DEFAULT_DOGFOOD
    g = build_gallery(path=source)
    owner_private = _owner_private_source(source)
    out_json = Path(json_output) if json_output else (
        OWNER_GALLERY_JSON if owner_private else PUBLIC_GALLERY_JSON
    )
    out_md = Path(markdown_output) if markdown_output else (
        OWNER_GALLERY_MD if owner_private else PUBLIC_GALLERY_MD
    )
    if owner_private:
        require_private_output(out_json, purpose="private gallery JSON")
        require_private_output(out_md, purpose="private gallery Markdown")

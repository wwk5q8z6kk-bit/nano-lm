"""Active Frontier status rollup — product eval snapshot (not Layer-1)."""
...
def build_status(*, corpus: Path | None = None, demo: bool = False) -> dict:
    ...
    admission = evaluate_admission(
        ...
        owner_corpus_contact=bool(corpus_path and owner_ready and owner_ready.get("ready_for_private_run")),
    )

    sla = measure_ingest_sla()
    return {
        "schema": "nano-lm.wedge_v1.frontier_status.v1",
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = argparse.ArgumentParser(description="Active Frontier status rollup")

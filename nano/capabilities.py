...
CAPABILITIES: tuple[Capability, ...] = (
    ...
    # ---------------- Evidence (grounding as its own domain) ----------------
    _c(capability_id="EVD-LOCATE", domain="Evidence",
       capability="Locate a claim in its source, modality-independently",
       internal_representation="EvidenceSpanV2 + Locator families",
       module="nano.contracts", inputs=("source", "claim"), outputs=("located span",),
       memory_requirements="source retained or referenced; never discarded",
       tools=(), training_objective="span localisation",
       evaluation_benchmark="evidence-span precision/recall (LCRB-1)",
       failure_modes=("span points at paraphrase rather than source text",
                      "offsets invalidated by later normalisation"),
       implementation_stage=Stage.A_ENCOUNTER, status=Status.IMPLEMENTED,
       evidence="nano/contracts.py::EvidenceSpanV2"),

    # ---------------- Conversation ----------------
    _c(capability_id="CNV-STATE", domain="Conversation",
       capability="Track dialogue state separately from world state",
       internal_representation="dialogue state (participants, goals, commitments)",
       module="not built", inputs=("turn", "history"), outputs=("intent", "dialogue act"),
       memory_requirements="conversation memory distinct from subject memory",
       tools=(), training_objective="dialogue-state tracking",
       evaluation_benchmark="correction uptake; clarification appropriateness",
       failure_modes=("dialogue state merged into world state",
                      "a user hypothetical recorded as a fact about the subject"),
       implementation_stage=Stage.C_PRESENTATION, status=Status.ABSENT),

    _c(capability_id="CNV-ACT", domain="Conversation",
       capability="Choose to answer, clarify, retrieve, show, cite, or abstain",
       internal_representation="dialogue act conditioned on uncertainty",
       module="not built", inputs=("intent", "uncertainty"), outputs=("dialogue act",),
       memory_requirements="none beyond dialogue state",
       tools=(), training_objective="act selection",
       evaluation_benchmark="does it clarify when the question is ambiguous?",
       failure_modes=("answers a question it should have clarified",
                      "asks for clarification it could have retrieved"),
       implementation_stage=Stage.C_PRESENTATION, status=Status.ABSENT),
    ...
    _c(capability_id="MTA-WOULDCHANGE", domain="Metacognition",
       ...
       internal_representation="falsifier attached to a claim",
       module="not built", inputs=("claim", "evidence"), outputs=("falsifier",),
       memory_requirements="none", tools=(),
       training_objective="falsifier generation",
       evaluation_benchmark="is the named falsifier actually decisive?",
       failure_modes=("names a falsifier that would change nothing",
                      "no falsifier, so the claim is unfalsifiable"),
       implementation_stage=Stage.D_LONGITUDINAL, status=Status.ABSENT),

    # ---------------- Prediction ----------------
    _c(capability_id="PRD-SEPARATE", domain="Prediction",
       capability="Keep observed / inferred / predicted / simulated strictly apart",
       internal_representation="derivation mode, an axis distinct from epistemic status",
       module="named in nano/ontology.py as DerivationMode; not built",
       inputs=("statement",), outputs=("derivation mode",),
       memory_requirements="mode travels with the statement",
       tools=(), training_objective="none (structural)",
       evaluation_benchmark="zero predicted statements rendered as observed",
       failure_modes=("a forecast presented as a finding (safety)",
                      "simulation output entering the ledger as evidence"),
       implementation_stage=Stage.E_HIGHER_RISK, status=Status.ABSENT),

    # ---------------- Artifact compilation ----------------
    _c(capability_id="ART-PLAN", domain="Artifact compilation",
       capability="Plan an artifact before writing it (audience, scope, evidence, format)",
       internal_representation="artifact specification preceding generation",
       module="not built", inputs=("state", "goal", "audience"),
       outputs=("artifact spec",),
       memory_requirements="none", tools=(),
       training_objective="specification quality",
       evaluation_benchmark="does the artifact satisfy its own spec?",
       failure_modes=("generation begins before scope is decided",
                      "many independent summarisers instead of one planner"),
       implementation_stage=Stage.C_PRESENTATION, status=Status.ABSENT),

    # ---------------- Human interaction ----------------
    _c(capability_id="HUM-AUDIENCE", domain="Human interaction",
       capability="Model the recipient without changing the underlying facts",
       internal_representation="user model held separately from the world model",
       module="not built", inputs=("state", "recipient"), outputs=("presentation",),
       memory_requirements="user model persists per recipient",
       tools=(), training_objective="audience adaptation",
       evaluation_benchmark="facts held constant across audiences",
       failure_modes=("simplification for a lay reader drops a critical fact",
                      "user model leaks into the world model"),
       implementation_stage=Stage.C_PRESENTATION, status=Status.ABSENT),
)
...
REQUIRED_DOMAINS = (
    ...
    "Evidence", "Conversation", "Metacognition", "Prediction",
    "Artifact compilation", "Human interaction",
)
